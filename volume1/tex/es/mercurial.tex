...
Esta representación consiste de tres tipos de revlogs: \emph{changelog}, \emph{manifests}, y
...
modelo actual está orientado verticalmente (un filelog por archivo), los 
desarrolladores de Mercurial a menudo se encontraron en la situación en la que 
...
Una de las gracias de la forma en que los revlogs están dispuestos es el
orden. Al ordenar los adiciones de manera tal que los filelogs sean añadidos al
principio, luego el manifest y finalmente el changelog, el repositorio siempre 
permanece en un estado consistente. Cualquier proceso que comience leyendo el 
changelog puede estar seguro que todos los punteros a otros revlogs son válidos, 
lo que soluciona algunos problemas en este departamento.
Sin embargo, Mercurial tiene además locks explícitos que aseguran que no hayan 
dos procesos agregando a los revlogs en paralelo.
...
La última estructura de datos importante es lo que llamamos el \emph{dirstate}.
El dirstate es una representación de lo que hay en el directorio de trabajo 
en cualquier momento. Pero lo más importante, es que mantiene un registro de
que revisión ha sido obtenida (checked out). La que sirve de base para todas las
comparaciones de los comandos \code{status} y \code{diff}, y también determina 
el o los padres del siguiente grupo de cambios a commitear. El dirstate tendrá 
dos padres cada vez que el comando \code{merge} se ejecuta, tratando de fusionar
un grupo de cambios con otro.
...
Dado que \code{status} y \code{diff} son operaciones muy comunes (ellas ayudan
a verificar el progreso de lo que uno tiene en relación al último cambio), el
dirstate también contiene un caché del estado del directorio de trabajo tal como 
estaba la última vez que Mercurial lo recorrió. Mantener el registro de las 
fechas de las últimas modificaciones y tamaños de los archivos permite acelerar 
el análisis. También necesitamos conocer el estado del archivo: si fue agregado,
eliminado, o fusionado en el directorio de trabajo. Esto permite también acelerar
el análisis del directorio de trabajo, y hace más fácil obtener la información al
momento del commit.
...
\begin{aosasect1}{Mecánica del Versionamiento}
%\begin{aosasect1}{Versioning Mechanics}

Ahora que ya está familiarizado con el modelo de datos subyacente y con la 
estructura del código en el más bajo nivel en Mercurial, consideremos ahora como
Mercurial implementa los conceptos de control de versiones sobre las bases descritas 
...
\begin{aosasect2}{Branches (Ramas)}
...
\begin{aosasect1}{Estructura General}
