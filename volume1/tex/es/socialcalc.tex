...
programa de hoja de cálculo, VisiCalc, fue creado por Dan Bricklin en 
1978 y entregado en 1979. El concepto original fue muy sencillo: una 
tabla que se expande infinitamente en dos dimensiones, sus celdas 
conteniendo texto, números, y fórmulas. Las fórmulas se componen de 
operadores aritméticos normales y de varias funciones incorporadas, y 
...
A pesar de que esa similitud era simple, esta tuvo muchas aplicaciones:
contabilidad, inventarios, y gestión de listas son solo algunas. Las 
posibilidades fueron prácticamente ilimitadas. Todos estos usos 
convirtieron a VisiCalc en la primera 'aplicación asesina' de la era del 
computador personal.
...
En las décadas siguientes, los sucesores como Lotus 1-2-3 y Excel 
hicieron mejoras incrementales, sin embargo, la esencia era la misma. La 
mayoría de las hojas de cálculo se almacenaban en archivos guardados en 
un disco, y se cargaban en la memoria cuando se abrían para el trabajo de 
edición. Particularmente, la colaboración fue difícil en el modelo basado 
en archivos:

\begin{aosaitemize}

%  \item Each user needed to install a version of the spreadsheet editor.
  \item Cada usuario necesitaba instalar una versión del editor de hoja 
  de cálculo.

%  \item E-mail ping-pong, shared folders, or setting up a dedicated
%  version-control system all added bookkeeping overhead.
  \item El ping-pong de los e-mails, las carpetas compartidas, o el ajuste 
  de un sistema de control de versiones saturaban el trabajo de contabilidad.

%  \item Change tracking was limited; for example, Excel does not preserve
%  history for formatting changes and cell comments.
  \item El seguimiento a los cambios era limitado; por ejemplo, Excel no 
  preserva el historial para los cambios de formato y los comentarios de 
  celdas.
...
%  template.
  \item Actualizar el formato o las fórmulas en plantillas requería de cambios 
  muy detallistas cada uno de los archivos de hoja de cálculo que usaran esa
  plantilla.
...
Afortunadamente, un nuevo modelo de colaboración surgió para manejar estos 
problemas con una simplicidad elegante. Este es el modelo wiki, inventado 
por Ward Cunningham en 1994, y popularizado por Wikipedia a comienzos de 
la década de 2000.
...
En vez de archivos, el modelo wiki se destaca por páginas hospedadas en 
servidores, editables en el explorador sin requerir ningún software especial. 
Esas páginas de hipertexto pueden enlazarse fácilmente una con otra, y aún 
más, incluir fragmentos de otras páginas para formar una página más grande. 
Todos los participantes ven y editan por defecto la versión más reciente, 
con un historial de revisión automáticamente administrado por el servidor.
...
Inspirado por el modelo wiki, Dan Bricklin comenzó a trabajar en WikiCalc 
en 2005. Su objetivo es combinar la facilidad de creación y de edición 
multipersonal de los wikis con la similitud de formato visual y 
de cálculo de las hojas de cálculo.
...
%time:
La primera versión de WikiCalc (\aosafigref{fig.soc.screenshot}) tuvo varias 
características que la distinguió de otras hojas de cálculo de ese momento:

\begin{aosaitemize}

%  \item Plain text, HTML, and wiki-style markup rendering for text data.
  \item Visualización de datos de texto en texto plano, en HTML y en un estilo 
  de etiquetas wiki.

%  \item Wiki-style text that includes commands to insert links, images,
%  and values from cell references.
  Estilo de texto wiki que incluye comandos para insertar enlaces, imágenes y 
  valores desde referencias de celdas.

%  \item Formula cells may reference values of other WikiCalc pages
%  hosted on other websites.
  Las celdas fórmulas pueden referenciar valores de otras páginas WikiCalc 
  alojadas en otros sitios web.

%  \item Ability to create output to be embedded in other web pages, both
%  static and live data.
  \item Habilidad para crear resultados para ser embebidos en otras páginas web, 
  sin importar si son datos estáticos o dinámicos.

%  \item Cell formatting with access to CSS style attributes and CSS
%  classes.
  \item Formato de celdas con acceso a atributos de estilo CSS y clases CSS.

%  \item Logging of all edit operations as an audit trail.
  \item Registro log de todas las operaciones de edición y que sirve para tareas 
  de auditoría.

%  \item Wiki-like retention of each new version of a page with roll-back
%  capability.
  \item Conservación al estilo Wiki de cada nueva versión de una página y con 
  capacidad de volver al estado anterior.
...
La arquitectura interna de WikiCalc 1.0 (\aosafigref{fig.soc.comp}) y el 
flujo de información (\aosafigref{fig.soc.flow}) fueron deliberadamente 
simples, pero poderosas. La habilidad para componer una hoja de cálculo 
maestra a partir de varias hojas de hojas de cálculo más pequeñas fue 
manejable. Por ejemplo, imagina un escenario donde cada vendedor guarda 
números en una página de la hoja de cálculo. Después, cada jefe de ventas 
une esos números en una hoja de cálculo regional, y luego el VP de las 
ventas une los números regionales en una hoja de cálculo de alto nivel.
...
Cada vez que una de las hojas de cálculo individuales es actualizada, todas 
las hojas de cálculo dependientes pueden reflejar la actualización. Si alguien 
quiere mayores detalles, simplemente hace click a través de la vista de 
la hoja de cálculo detrás de la hoja de cálculo. Esta capacidad enrollamiento 
elimina el esfuerzo redundante y sujeto a errores de la actualización de números en 
múltiples lugares, y asegura que todas las vistas de la información permanezcan 
recientes. 
...
Para asegurarse que los nuevos cálculos están al día, WikiCalc adoptó 
un diseño de cliente ligero, manteniendo todo el estado de la información 
en el lado del servidor. Cada hoja de cálculo se representó en el explorador 
como un elemento \code{{\textless}table{\textgreater}}; editar una celda 
enviará una llamada \code{ajaxsetcell} al servidor, y después el servidor 
le dirá al explorador cuáles celdas necesitan actualización.
...
Sin ser sorpresa, este diseño depende de una conexión rápida entre el 
explorador y el servidor. Cuando la latencia es alta, los usuarios comenzarán 
a notar que la apariencia frecuente de mensajes ``Cargando\ldots'' entre 
actualizar una celda y ver sus nuevos contenidos como se muestra en \aosafigref{fig.soc.load}. Este es un problema especialmente para usuarios 
que editan interactivamente fórmulas cuando ingresan datos y esperan para ver 
...
%size of pages.
Además, ya que el elemento \code{{\textless}table{\textgreater}} tiene las mismas 
dimensiones que la hoja de cálculo, una grilla de tamaño $100{\times}100$ 
crearía 10,000 elementos \code{{\textless}td{\textgreater}} DOM, lo cual 
restringe el recurso de memoria de los exploradores, limitando aún más el tamaño 
de las páginas.
...
Debido a estas deficiencias, mientras WikiCalc que era útil como un 
servidor solitario ejecutándose un computador local, no era muy práctico 
embeberlo como parte de sistemas gestores de contenido basados en web.
...
En 2006, Dan Bricklin se asoció con SocialText para iniciar el desarrollo de 
Social, una re-escritura desde las bases de WikiCalc en Javascript basada en algo 
del código Perl original.
...
%goals included:
Esta re-escritura se dirigió a colaboraciones grandes y distribuidas, y buscada para 
entregar una apariencia más cercana a la de una aplicación desktop. Otros objetivos de
diseño incluyeron:

\begin{aosaitemize}

%  \item Capable of handling hundreds of thousands of cells.
  \item Capaz de manejar cientos de miles de celdas.

%  \item Fast turnaround time for edit operations.
  Rápido tiempo de cambio de turnos para editar operaciones.

%  \item Client-side audit trail and undo/redo stack.
  Rastro de auditoría y pila deshacer/rehacer en el lado del cliente.

%  \item Better use of Javascript and CSS to provide full-fledged layout
%  functionality.
  Un mejor uso de Javascript y CSS para proveer una funcionalidad completa.

%  \item Cross-browser support, despite the more extensive use of
%  responsive Javascript.
  Soporte multi-explorador, a pesar del mayor uso extensivo de Javascript responsivo.
...
%system.
Después de tres años de desarrollo y varios lanzamientos beta, SocialText lanzó 
SocialCalc 1.0 en 2009, cumpliendo exitosamente los objetivos de diseño. Ahora 
démosle una mirada a la arquitectura del sistema SocialCalc.
...
\aosafigref{fig.soc.action} y \aosafigref{fig.soc.class} muestran respectivamente 
la interfaz de SocialCalc y las clases. Comparado con WikiCalc, el rol del servidor 
se redujo bastante. Su Única responsabilidad es responder los GETs HTTP sirviendo 
hojas de cálculo completas y serializadas en formato de guardado; una vez los 
exploradores reciben los datos, todos los cálculos, gestión de cambio e interacción con el usuario son implementados en Javascript.
...
%aspect:
Los componentes Javascript se diseñaron con un estilo por capas MVC (Model/View/Controller. En inglés, Modelo, Vista, Controlador), con cada clase 
enfocada en solo aspecto:
...
%  Empty cells need no entries, and hence consume no memory at all.
  \item{\emph{Sheet}} es el modelo de datos representando una estructura 
  en memoria de la hoja de cálculo.
  Esta contiene un diccionario de coordenadas hacia objetos \emph{Cell}, en 
  donde cada coordenada representa solo a una celda.
  Las celdas vacías no necesitan estar en el diccionario, y por tanto, no 
  consumen nada de memoria.
...
%  common properties are shown in \aosatblref{tbl.soc.cellcontents}.
  \item{\emph{Cell}} representa el contenido y el formato de una celda. 
  Algunas propiedades comunes se muestran en \aosatblref{tbl.soc.cellcontents}.

%  \item{\emph{RenderContext}} implements the view; it is responsible
%  for rendering a sheet into DOM objects.
  \item{\emph{RenderContext}} implementa la vista; es responsable de convertir  
  una hoja en objetos DOM.
...
%  content, it schedules new commands to the sheet's command queue
  \item{\emph{TableControl}} es el controlador principal que procesa 
  los eventos de mouse y de teclado.
  Ya que este controlador captura los eventos de la vista, tales como 
  desplazamiento y cambio de tamaño, actualiza su objeto \emph{RenderContext} 
  asociado. Mientras captura eventos de actualización que afecten 
  el contenido de la hoja, planifica nuevos comandos en la cola de 
  comandos de la hoja.

%  \item{\emph{SpreadSheetControl}} is the top-level UI with
%  toolbars, status bars, dialog boxes and color pickers.
  \item{\emph{SpreadSheetControl}} es la UI de alto nivel (User Interface. 
  En inglés,   Interfaz de Usuario) con barras de herramientas, barras 
  de estado, cuadros de diálogo y selectores de color.

%  \item{\emph{SpreadSheetViewer}} is an alternate top-level UI that
%  provides a read-only interactive view.
  \item{\emph{SpreadSheetViewer}} es una UI alterna de alto nivel que 
  provee una vista interactiva de sólo lectura.
...
Adoptamos una mínimo sistema de objetos basados en clases con 
composición/delegación simples, sin usar herencia ni prototipos de 
objetos. Todos los símbolos fueron organizados dentro del espacio 
de nombres \code{SocialCalc.*} para evitar conflictos de nombres.
...
Cada actualización en la hoja se procesa a través del método \code{ScheduleSheetCommands}, el cual toma un string de comando que 
representa dicha actualización. (Algunos comandos comunes se muestran en \aosatblref{tbl.soc.commands}.) La aplicación que embebe a SocialCalc puede 
definir comandos extras por su cuenta si agrega callbacks (re-llamadas) 
dentro del objeto \code{SocialCalc.SheetCommandInfo.CmdExtensionCallbacks}, 
y usa el comando \code{startcmdextension} para invocar esos comandos adicionales.
...
%command queue.
Para mejorar el nivel de respuesta, SocialCalc hace todo el re-cálculo y 
todas las actualizaciones DOM en segundo plano, así que el usuario 
puede continuar haciendo cambios en varias celdas mientras el motor de 
SocialCalc procesa y se pone al día con los cambios previos almacenados 
...
Cuando un comando se está ejecutando, el objeto \code{TableEditor} 
establece a verdadero su indicador \code{busy}; los comandos posteriores 
se ponen en la cola \code{deferredCommands}, asegurando un orden de 
ejecución secuencial. Como el diagrama de ciclo de eventos \aosafigref{fig.soc.loop} 
muestra, el objeto Hoja se mantiene enviando eventos \code{StatusCallback} 
para notificar al usuario del estado actual de la ejecución de comandos, 
siguiendo cada uno de estos cuatro pasos:
...
%   to the \emph{PositionCalculations} step.
   \item{\emph{ExecuteCommand}}: Envía \code{cmdstart} al iniciar la 
   ejecución del comando, y \code{cmdend} al terminar su ejecución. Si 
   el comando cambia indirectamente el valor de una celda, entonces el 
   se continua con el paso \emph{Recalc}.
   En cambio, si el comando cambió la apariencia visual de una o 
   mas celdas mostradas en la pantalla, se continua con el paso \emph{Render}. 
   Si ninguno de estos cambios ocurre (por ejemplo, el comando 
   \code{copy}), entonces se continua con el paso \emph{PositionCalculations}.
...
%  step.
  \item{\emph{Recalc}} \emph{(as{ }needed)}: Envía \code{calcstart} al iniciar, 
  \code{calcorder} cada 100 ms al revisar la cadena de dependencia de las 
  celdas, \code{calccheckdone} al terminar la revisión, y \code{calcfinished} 
  cuando todas las celdas afectadas reciban sus valores re-calculados. Después 
  de este paso, siempre se continua con el paso \emph{Render}.
...
%  formatted cells. This step is always followed by \emph{PositionCalculations}.
  \item{\emph{Render}} \emph{(as{ }needed)}: Envía \code{schedrender} al iniciar, 
  y \code{renderdone} cuando el elemento \code{{\textless}table{\textgreater}} 
  se actualiza con las celdas con formato. Después de este paso, siempre se 
  continua con el paso \emph{PositionCalculations}.
...
%  \code{TableEditor}.
  \item{\emph{PositionCalculations}}: Envía \code{schedposcalc} al iniciar, 
  y \code{doneposcalc} después de actualizar las barras de desplazamiento, 
  al cursor actual de la celda editable, y a otros componentes visuales del  
  \code{TableEditor}.
...
Ya que todos los comandos se guardan al ser ejecutados, naturalmente 
obtenemos un log de auditoría de todas las operaciones. El método 
\code{Sheet.CreateAuditString} provee un string delimitado con una 
nueva línea que sirve como rastro de auditoría, con cada comando 
en una línea sencilla y diferente.
...
%original value.
\code{ExecuteSheetCommand} tambiÃ©n crea un comando deshacer para cada 
comando que ejecuta. Por ejemplo, si la celda A1 contiene ``Foo'' y 
el usuario ejecuta \code{set A1 text Bar}, entonces un comando deshacer 
\code{set A1 text Foo} se pone en la pila de deshacer. Si el usuario 
hace click en Deshacer, entonces el comando deshacer se ejecuta para 
restaurar A1 a su valor original.
...
%instances.
Ahora miremos la capa TableEditor. Esta calcula las coordenadas en 
pantalla de su \code{RenderContext}, y gestiona las barras de desplazamiento 
horizontal y vertical usando dos instancias \code{TableControl}.
...
La capa de vista, manejada por la clase \code{RenderContext}, también 
se diferencia del diseño de WikiCalc. En lugar de mapear cada celda a 
un elemento \code{{\textless}td{\textgreater}}, simplemente creamos 
un \code{{\textless}table{\textgreater}} de tamaño fijo  que se ajusta 
al área visible del explorador, y la pre-llenamos con sus elementos  \code{{\textless}td{\textgreater}}.
...
A medida que el usuario desplaza la hoja de cálculo a través de nuestras 
barras de desplazamiento dibujadas a preferencia, actualizamos dinámicamente 
el \code{innerHTML} de los elementos \code{{\textless}td{\textgreater}} 
pre-dibujados. Esto significa que no necesitamos crear o destruir ningún 
elemento \code{{\textless}tr{\textgreater}} ni \code{{\textless}td{\textgreater}} 
en muchos casos comunes, lo cual acelera el tiempo de respuesta.
...
%performance.
A causa de que \code{RenderContext} solo da la región visible, el tamaño 
del objeto Hoja puede ser arbitrariamente grande sin afectar su rendimiento.
