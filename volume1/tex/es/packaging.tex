...
Hay dos escuelas de pensamiento cuando se trata de la instalación de aplicaciones.  
La primera, común en Windows y Mac OS X, es que las aplicaciones deben ser 
auto-contenidas(TODO), y su instalación no debería depender de nada más.  Esta 
filosofía simplifica la gestión de applicaciones: cada aplicación es un ``dispositivo''
independiente (REVIEW), su instalación y eliminación no debería perturbar al resto del 
sistema operativo\@.  Si la aplicación necesita una librería no común, esa librería
debería estar contenida en la distribución de la aplicación.
...
La segunda escuela, que es la norma para los sistemas basados​en Linux, trata
al software(TODO) como una colección de pequeñas unidades independientes llamadas
\emph{paquetes}. Las librerías están agrupadas en paquetes, cualquier paquete de 
librerías podría depender de otros paquetes.
La instalación de una aplicación podría implicar la búsqueda y la instalación de docenas
de versiones particulares de otras librerías.  Estas dependencias generalmente se
obtienen de un repositorio central que contiene miles de paquetes.  Esta filosofía,
es la razón por la cual distribuciones de Linux utilizan complejos sistemas de gestión
de paquetes como \code{dpkg} y \code{RPM} para realizar el seguimiento de las
dependencias y evitar la instalación de dos aplicaciones que utilicen versiones
incompatibles de la misma librería.
...
Hay pros y los contras de cada enfoque.  Tener un sistema altamente modular donde cada
pieza puede ser actualizada o reemplazada hace que la gestión más fácil, ya que cada
librería está presente en un solo lugar, y todas las aplicaciones que la utilizan se
benificiarán cuando es actualiza.  Por ejemplo, una revisión de seguridad en una
biblioteca en particular llegará a todas las aplicaciones que la utilizan a la vez, 
mientras que si una aplicación distribuye su propia librería, esa revisión de seguridad
será más compleja de implementar, especialmente si las distintas aplicaciones utilizan
diferentes versiones de la librería.
