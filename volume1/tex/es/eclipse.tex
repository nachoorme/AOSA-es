...
La implementación de la modularidad en el software es una tarea notoriamente difícil.
La interoperabilidad una gran cantidad de código fuente escrito por una
...
promover la cohesión y crecimiento de la comunidad? Retrocedamos
al principio.
...
todo y nada en particular.'' Esta descripción fue genérica adrede,
...
promovió que la comunidad desarrolle a partir de una plataforma núcleo
...
corregir "bugs" e implementar nuevas y fantásticas características
...
soporte de construcción de la aplicación, y mucho más. Cada uno de
...
intención era que tales herramientas de desarrollo servirían como
...
A fin de incentivar a las personas a desarrollar a partir de la
...
funcionalidad relacionada contribuida por plugins en las primeras
...
El workbench es el elemento de interfaz de usuario (UI) más familiar
...
es la vista ``problemas'' que indica errores o alertas en el código
...
era "pixel perfect" y la sensación general fue, ``no puedo crear
...
Los primeros SDK de Eclipse se ejecutaron el Linux y Windows. Para
...
Java puro y no contiene ningún código nativo a plataforma.
...
agrupados juntos en tablas de contenidos (TOCs.) Considérese los
...
soporte a un constructor incremental dentro del IDE\@. Un constructor
incremental proporciona mejor rendimiento pues sólo recompila archivos
que han cambiado o sus dependencias. Cómo trabaja el constructor
...
Un constructor dentro de Eclipse toma archivos de entrada
...
proyecto. Adicionalmente a los requerimientos técnico, también era
...
proporcionó un nuevo editor de manifiesto dentro de Eclipse. Cada
...
compilador Java no entiende bundles ni manifiestos OSGi. El PDE
...
J2SE-1.4 en el manifiesto, y entonces se escribe algún código que 
...
bundle tiene su propio classloader. La ruta de clases (classpath) disponible a un bundle
se construye examinando las dependencias en el manifiesto y generando un classpath
disponible para el bundle. Las aplicaciones OSGi son conjuntos de bundles. Para
...
\aosafigure[200pt]{../images/eclipse/bundlelifecycle.eps}{Ciclo de Vida de Bundle OSGi}{fig.ecl.blc}

OSGi es un framework dinámico que soporta la instalación, inicialización, detención y detención de bundles.
Como se mencionó antes, la activación diferida (lazy activation) fue una ventaja central a Eclipse pues las
clases de plugins no se cargaban hasta que se necesiten. El ciclo de vida del bundle OSGi también permite
...
ser cargadas y ejecutadas. El estado ``iniciando'' (starting) significa que el bundle está siendo activado de acuerdo
con su política de activación. Una vez activado, el bundle está en estado activo (active); éste puede adquirir recursos
requeridos e interactuar con otros bundles. Un bundle está en estado ``deteniéndose'' (stopping) cuando está
...
Adicionalmente a expresar las dependencias fijas entre bundles, también hay un mecanismo dentro de OSGi
llamado servicios que proporciona mayor desacoplamiento entre bundles. Los servicios son objetos con un
...
Como ejemplos de aplicaciones RCP podemos mencionar su uso para
monitorear los robots Mars Rover desarrollado por NASA en el
...
a continuar extendiendo Eclipse a sus propios IDEs, los consumidores puede construir
a partir del framework de aplicaciones RCP para plataformas más genéricas.
...
Ud. puede ver que el fragmento \code{org.eclipse.launcher.gtk.linux.x86\_64} especifica
...
\aosafigure{../images/eclipse/p2.eps}{Conceptos en P2}{fig.ecl.p2}
%\aosafigure{../images/eclipse/p2.eps}{P2 Concepts}{fig.ecl.p2}

Un perfil es una lista de IUs en una instalación. Por ejemplo, su
SDK Eclipse tiene un perfil que describe su instalación actual. Desde
el mismo Eclipse, Ud. puede solicitar una actualización a una nueva
...
embargo, los usuarios fueron con frecuencia pasaron por alto estos asuntos
...
ser alterado para generar metadata ya sea al construir o ejecutar
...
el el diseño de una nueva versión de Eclipse. Al mismo tiempo,
...
Eclipse fue lanzado el 2001, antes de la era de aplicaciones
Internet de apariencia enriquecida que pudiera ser maquillada
...
sin tener que tener un conocimiento profundo de todas las APIs
disponibles. Son estructuradas como servicios individuales
tal que pueden ser utilizadas también en otros lenguajes además
de Java, tales como Javascript. Por ejemplo, hay un API
para acceder el modelo de aplicación, para leer y modificar preferencias
...
nuestro expansivo API resulta difícil de adoptar y entender para
...
las razones por la que se creó el canal Eclipse 4.x.
...
y responde preguntas de la comunidad. A través del tiempo esta
...
consumidores, manteniéndolos unidos en la comunidad. Si estás interesado(a)
en participar de Eclipse, por favor visita http://www.eclipse.org.
